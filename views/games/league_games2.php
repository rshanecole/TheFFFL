...
?>
	<script type="text/javascript">
		function load_league_games(){
				...
				$.ajax({
					type: "POST",
					url: "<? echo base_url(); ?>Game/league_games2/<? echo $league_id; ?>/<? echo $year; ?>/<? echo $week; ?>",
					async: false
						...
						$.each(["a","b"], function(letter_index, letter){
							...
							$.each(val["starters_"+letter], function( fffl_player_id, player_data ){
								...
								if (~player_data["status"].indexOf("Final")){
									...
									$("#"+game+"_team_"+letter+"_"+player+"_background").removeClass("bg-danger bg-info bg-warning");
    ...
    <div id="test_area"></div>
		...
		while($game_priority < $number_games){  
			$game_priority++;
			if($game_priority==1) { $panel_color='red'; } else { $panel_color='blue'; } ?>
                                ...
                                <table class="table table-hover table-condensed col-xs-24 hidden-xs roster_<? echo $game_priority; ?>" id="">
                                    <tbody id = "<? echo $game_priority; ?>_team_<? echo $letter; ?>_roster"> 
                                        ...
                                        <? $player_count++;
                                            ...
                                            <div class="row">
												<div class="text-center col-xs-24">
                                                	<span class="pointer" onClick=" $(this).parent().parent().next().toggle(); $(this).toggle(); $(this).next().toggle(); "><small>View Bench</small></span>
                            						<span class="pointer" onClick="$(this).parent().parent().next().toggle(); $(this).toggle();  $(this).prev().toggle(); " style="display:none"><small>Hide Bench</small></span>
                            					</div>
											</div>
                                            
                                        <? //bench players 
										 ?>
				<table class="table table-hover table-condensed col-xs-24 hidden-xs" style="display:none">
