    ...
    <div style="" id="open_player" data-player="0" data-team="0"></div>
		<div id="week_selector" class="col-xs-24 page_title text-center " style="padding-bottom:5px;">
            ...
            <button class="btn btn-default dropdown-toggle" type="button" id="group_dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                <span id="dropdown_group">
                	<? if($week<$first_playoff){?>
                		Week <? echo $week; ?>
                	<? }
					elseif($week<$superbowl_week){ ?>
                    	Playoffs
                	<? }
					elseif($week==$superbowl_week){ ?>
                    	Superbowl
            ...
            <ul class="dropdown-menu"  aria-labelledby="dropdownMenu1" >
                ...
                while($w<=17){ 
                    ...
                    <? }
                    elseif($w<$superbowl_week){
					?>
                    	<li><a href="#" onClick="change_content('<? echo base_url()."Game/week/".$league_id."/".$year."/".$w; ?>',$('#dropdown_title').html())" >Playoffs</a></li>
					<? }
					elseif($w==$superbowl_week){ ?>
						<li><a href="#" onClick="change_content('<? echo base_url()."Game/week/".$league_id."/".$year."/".$w; ?>',$('#dropdown_title').html())" >Superbowl</a></li>	
					<? }
					else {
					?>
                    	<li><a href="#" onClick="change_content('<? echo base_url()."Game/week/".$league_id."/".$year."/".$w; ?>',$('#dropdown_title').html())" >Prowbowl</a></li>
					<? } ?>
                <? $w++; 
                } ?>
            </ul>
